...
Flight phases:
  ...
  -> TOP_APPROACH(to 6.8m) -> HOVER(5s) -> FLY_LEFT(5s) -> STABILIZE(3s)
  -> FLY_RIGHT(5s) -> STABILIZE(3s) -> DESCEND(to 2m)
  -> RAMP_APPROACH(to 0.5m) -> LOW_HOVER(10s) -> LAND(contact-based) -> DISARM
...
GROUND_CONTACT_ALT = 0.10
GROUND_CONTACT_VZ = 0.2
GROUND_CONTACT_HOLD = 0.5
LAND_TIMEOUT = 15.0
...
def update_phase(state: TestState, t: float, dt: float = 0.001):
    ...
    if phase == Phase.BOOT:
        ...
        if elapsed >= ALTHOLD_SETTLE:
            print(f"[{t:6.1f}s] [  ENABLE_ALTHOLD] ALT_HOLD engaged.")
        ...
        if state.ground_contact_time >= GROUND_CONTACT_HOLD:
            ...
            print(
                f"[{t:6.1f}s] [          LAND] "
                f"Ground contact: alt={state.current_altitude:.2f}m vz={state.current_vz:.2f}m/s"
            )
            state.transition(Phase.DISARM, t)
...
def print_results(state: TestState):
    ...
    if state.land_altitude > GROUND_CONTACT_ALT:
        passed = False
        issues.append(f"Did not land properly ({state.land_altitude:.2f}m > {GROUND_CONTACT_ALT}m)")
